...
public class LoginController {
  ...
  //  @PostMapping("/login")
    public String loginV3(@Valid @ModelAttribute LoginForm loginForm, BindingResult bindingResult, HttpServletRequest request){
        ...
        //true: (세션이 있으면 기존 세션 반환. 세션이 없으면 새로운 세션 생성해서 반환)
        //false: 세션이 있으면 기존 세션 반환. 세션이 없으면 새로운 세션을 생성하지 않음. null 반환함.
        //세션에 로그인 회원정보 보관
    ...
    @PostMapping("/login")
    public String loginV4(@Valid @ModelAttribute LoginForm login, BindingResult bindingResult,
  ...
  //  @PostMapping("/logout")
    ...
    public String logoutV3(HttpServletRequest request){
        HttpSession session = request.getSession(false);    //있는 세션을 가져오고
        if(session!=null){
            session.invalidate();   //세션+그 안에 데이터가 다 없어짐
        }
        return "redirect:/";
    }
